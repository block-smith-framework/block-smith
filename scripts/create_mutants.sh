...
#
# Run project
# Usage: bash create_mutants.sh <fragment-url> <blocktest-dir> <output-dir>
#
...
check_inputs() {
	if [[ ! -d ${BLOCKTEST_DIR} || -z ${OUTPUT_DIR} ]]; then
		echo "Usage: bash create_mutants.sh <fragment-url> <blocktest-dir> <output-dir>"
		exit 1
	fi
...
run() {
	...
	local start=$(epoch_ms)
	local maven_home=$(mvn --version | grep "Maven home" | cut -d ' ' -f 3)
	local block_gen_dir=$(realpath ${SCRIPT_DIR}/..)
	
	java -Dmaven.home=${maven_home} -jar ${SCRIPT_DIR}/../blockgen/target/blockgen-1.0.jar mutants ${PROJECT_DIR} ${TARGET_FILE} ${OUTPUT_DIR}/mutation ${block_gen_dir} ${START_LINE} ${END_LINE}
