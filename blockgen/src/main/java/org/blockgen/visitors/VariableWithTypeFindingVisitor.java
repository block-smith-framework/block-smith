...
public class VariableWithTypeFindingVisitor  extends VoidVisitorAdapter<Context>  {
    @Override
    public void visit(final AssignExpr n, final Context ctx) {
        System.out.println("VISIT AssignExpr : " + n);
        ctx.variableToAssignment.put(n.getTarget().toString(), n.getValue().toString());
        ...
        if (!isLocalVariable(target.toString(), ctx)) {
            ...
            if (target.isNameExpr() || (target.isFieldAccessExpr())) {
                ...
                try {
                    String targetTypeStr = TypeResolverUtil.calculateType(target);
                    ctx.assignedVariablesWithType.put(target.toString(), targetTypeStr);
                } catch (Exception ex) {
                    System.out.println("Unable to determine the type for " + target + ". Let's hope for the best");
                }

                if (!isPreviouslyAssigned(target.toString(), n)) {
                    System.out.println("Adding variable " + target + " to uninitializedVariables");
                    ctx.uninitializedVariables.add(target.toString());
                    ctx.uninitializedVariablesWithType.put(target.toString(), TypeResolverUtil.calculateType(target));
                }
            }
    ...
    public void visit(final VariableDeclarator n, final Context ctx) {
        ...
        if (n.getInitializer().isPresent()) {
            ctx.variableToAssignment.put(n.getNameAsString(), n.getInitializer().get().toString());
        }
        ...
        n.getInitializer().ifPresent(l -> {
            if (!isLocalVariable(n.getNameAsString(), ctx)) {
                ctx.logVariablesWithTypeAfter.put(n.getNameAsString(), n.getTypeAsString());
    ...
    public void visit(final FieldAccessExpr n, final Context ctx) {
        ...
        if (n.getScope().toString().equals("this")) { // TODO: Utils.getRootCaller(n)?
            System.out.println("FieldAccessExpr " + n + " is accessed via this, rewriting");
            ...
            if (!isPreviouslyAssigned(n.toString(), n)) {
                ...
                ctx.uninitializedVariablesWithType.put(n.toString(), TypeResolverUtil.calculateType(n));
            }
            return;
        } else if (ctx.logVariablesBefore.contains(scope.toString()) ||
        ...
        // First time see this field access

        System.out.println("FieldAccessExpr - " + n + ", " + containLocalVariable(n, ctx) + ", " + n.getNameAsString());
        if (containLocalVariable(n, ctx))
            return;

        if (SpoonResolver.setUpOK && System.getenv("OVERRIDE_STATIC_FIELDS") == null) {
            if (SpoonResolver.staticFieldsUsed.contains(n.toString()) || SpoonResolver.staticFieldsUsed.contains(scope.toString())) {
        ...
        if (!probablyNonStatic) {
            ...
            try {
                ...
                System.out.println(n + " is probably " + (probablyNonStatic ? "non-static" : "static") + " based on resolution");
            } catch (Exception ignored) {
        ...
        if (probablyNonStatic || System.getenv("OVERRIDE_STATIC_FIELDS") != null) {
    ...
    public void visit(final NameExpr n, final Context ctx) {
        ...
        boolean nameExprIsAClass = false;
        if (SpoonResolver.setUpOK && SpoonResolver.classesUsed.contains(name)) {
            System.out.println("Skipping: this NameExpr (" + name + ") is a class name");
            nameExprIsAClass = true;
        }

        if (!isLocalVariable(name, ctx) && (Utils.isStartWithLowerCase(name)
                || (!nameExprIsAClass && Utils.isConstant(name) && (name.length() >= 2 || isPreviouslyAssigned(n.toString(), n)) ))) {
            addVariablesBefore(n, ctx);
    ...
    public void visit(MethodCallExpr n, final Context ctx) {
        String name = n.getNameAsString();
        boolean shouldHandleThis = n.getArguments().stream().anyMatch(arg -> arg instanceof ThisExpr) && !ctx.canCreateThis;
        ...
        if (inPlainMethodCall != null) {
            ...
            // Method is enclosed in a plain method call
            System.out.println("inPlainMethodCall is " + inPlainMethodCall);
            System.out.println("inPlainMethodCall.getScope() is " + inPlainMethodCall.getScope());
            Optional<Expression> rootCaller = Utils.getRootCaller(inPlainMethodCall);
            System.out.println("Utils.getRootCaller(inPlainMethodCall) is " + rootCaller);
            if (rootCaller.isPresent()) {
                // outer call has explicit caller
                if (rootCaller.get().toString().equals("super") || rootCaller.get().toString().equals("this")) {
                    shouldMock = true;
                    shouldMockTarget = inPlainMethodCall;
                } else {
                    try {
                        String targetTypeStr = TypeResolverUtil.calculateType(rootCaller.get());
                        if (targetTypeStr.equals("org.slf4j.Logger")) {
                            // mock because we don't want to generate a logger object
                            System.out.println("Need to mock logger.");
                            shouldMock = true;
                            ...
                            ctx.invokedVariablesWithType.put(rootCaller.get().toString(), targetTypeStr);
                        }
                    } catch (Exception ignored) {}
                }
            } else {
                try {
                    if (!inPlainMethodCall.resolve().isStatic()) {
                        System.out.println("Outer call is not static, need to mock");
                        shouldMock = true;
                        shouldMockTarget = inPlainMethodCall;
                    }
                } catch (Exception ex) {
        ...
        // We want to log the method call (i.e., turn method call into a variable)
        // For example: bar = foo(1) + 2, and we want to turn foo(1) into an input variable
        // Turn method call into variable if
        // 1. Method is not called from an object (like o.m()) and method is not static and the return type is not void
        // 2. Method is call from `this`
        // 3. Method uses `this` in its parameter (unless we can replace this with constructor injection (if no argument))
        try {
            ResolvedMethodDeclaration resolved = n.resolve();
            boolean isStatic = resolved.isStatic();
            System.out.println("Is no root caller: " + !Utils.getRootCaller(n).isPresent());
            System.out.println("Is non void: " +  !n.calculateResolvedType().isVoid());
            System.out.println("Is non static: " + !isStatic);
            if ((!Utils.getRootCaller(n).isPresent() && !n.calculateResolvedType().isVoid() && !isStatic)
                    || (Utils.getRootCaller(n).isPresent() && Utils.getRootCaller(n).get().toString().equals("this")) || shouldHandleThis) {
                System.out.println("LOG METHOD CALL (confidence: high) " + n);
                addVariablesBefore(n, ctx);
                ctx.rename.add(n);
                ctx.givenMethods.add(n.toString());
            } else {
                super.visit(n, ctx);
            }
        } catch (Exception ignored) {
            ...
            if (SpoonResolver.setUpOK && SpoonResolver.methodCalls.containsKey(n.toString())) {
                ...
                boolean isStatic = info.isStatic;
                boolean isVoid = info.returnType.equals("void");

                System.out.println("Is no root caller: " + !Utils.getRootCaller(n).isPresent());
                System.out.println("Is non void: " +  !isVoid);
                System.out.println("Is non static: " + !isStatic);

                if ((!Utils.getRootCaller(n).isPresent() && !isVoid && !isStatic)
                        || (Utils.getRootCaller(n).isPresent() && Utils.getRootCaller(n).get().toString().equals("this")) || shouldHandleThis) {
                    System.out.println("LOG METHOD CALL (confidence: very high) " + n);
                    addVariablesBefore(n, ctx);
                    ctx.rename.add(n);
                    ctx.givenMethods.add(n.toString());
                } else {
            ...
            // TODO: band-aid?
            System.out.println("Unable to resolve " + n);
            if ((!Utils.getRootCaller(n).isPresent())
                    || (Utils.getRootCaller(n).isPresent() && Utils.getRootCaller(n).get().toString().equals("this")) || shouldHandleThis) {
                // Cannot check whether method is static or return type is non-void
                System.out.println("LOG METHOD CALL (confidence: low) " + n);
                addVariablesBefore(n, ctx);
                ctx.rename.add(n);
                ctx.givenMethods.add(n.toString());
            } else {
    ...
    public void visit(final TryStmt n, final Context ctx) {
        ...
        n.getCatchClauses().forEach(catchClause -> {
            ...
            variableSet.add(param.getNameAsString());
        });

        ctx.locals.push(variableSet);
        n.getCatchClauses().forEach(p -> p.accept(this, ctx));
    ...
    private void addVariablesBefore(Expression target, final Context ctx) {
        System.out.println("addVariablesBefore - Calculating type " + target);
        String targetTypeStr = TypeResolverUtil.calculateType(target);
    ...
    private void addVariablesAfter(Expression target, final Context ctx) {
        System.out.println("addVariablesAfter - Calculating type " + target);
        String targetTypeStr = TypeResolverUtil.calculateType(target);
