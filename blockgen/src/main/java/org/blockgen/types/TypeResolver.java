    ...
    public static void setup() {
        ...
        try {
            ...
            if (TypeResolverUtil.depClassPaths != null) {
                System.out.println("TypeResolver - setting up jar: " + TypeResolverUtil.depClassPaths);
            ...
            if (TypeResolverUtil.appSrcPath != null) {
                ...
                for (String path : TypeResolverUtil.appSrcPath.split(File.pathSeparator)) {
                    System.out.println("TypeResolver - adding path: " + path);
                    if (Paths.get(path).toFile().isDirectory()) {
                        sTypeSolver.add(new JavaParserTypeSolver(path));
                    } else {
                        // assume it is a file
                        if (TypeResolverUtil.appSrcPath.contains(Utils.getSourcePath())) {
                            sTypeSolver
                                    .add(new JavaParserTypeSolver(
                                            TypeResolverUtil.appSrcPath.split("src")[0] + Utils.getSourcePath()));
                        } else {
                            sTypeSolver
                                    .add(new JavaParserTypeSolver(
