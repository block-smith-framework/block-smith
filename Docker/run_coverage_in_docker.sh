...
# Usage: bash run_coverage_in_docker.sh [-e <exli-pipeline-ouput-dir> -g <genie> -k <kex> -j <jdoop> -d <jdart>] <projects-url-list> <output-dir> [branch=false] [timeout=86400 (sec)]
#
...
function check_input() {
  if [[ ! -f ${PROJECTS_LIST} || -z ${OUTPUT_DIR} ]]; then
    echo "Usage: run_coverage_in_docker.sh [-e <exli-pipeline-ouput-dir> -g <genie> -k <kex> -j <jdoop> -d <jdart>] <projects-url-list> <output-dir> [branch=false] [timeout=86400 (sec)]"
    exit 1
  fi
...
function run_project() {
  ...
  local id=$(docker run -itd --name bcov-${project_name} blocksmith:latest)
  ...
  echo "$(date) Finished getting coverage score for ${project_name} in ${duration} ms" |& tee -a docker.log
}
...
function process_technique() {
  ...
  if [[ ${technique} == "exli" || ${technique} == "INd" || ${technique} == "INs" || ${technique} == "INr" ]]; then
    # Kex, JDoop, and JDart uses Genie 
  ...
  if [[ -n ${tests_file} && -n ${dot_blocktests} && -n ${pipeline_output_dir} ]]; then
    ...
    timeout ${TIMEOUT} docker exec -w /home/blockgen/block-gen/scripts -e M2_HOME=/home/blockgen/apache-maven -e MAVEN_HOME=/home/blockgen/apache-maven -e CLASSPATH=/home/blockgen/aspectj-1.9.7/lib/aspectjtools.jar:/home/blockgen/aspectj-1.9.7/lib/aspectjrt.jar:/home/blockgen/aspectj-1.9.7/lib/aspectjweaver.jar: -e PATH=/home/blockgen/.local/bin:/home/blockgen/apache-maven/bin:/usr/lib/jvm/java-8-openjdk/bin:/home/blockgen/aspectj-1.9.7/bin:/home/blockgen/aspectj-1.9.7/lib/aspectjweaver.jar:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin ${id} timeout ${TIMEOUT} bash run_coverage.sh ${url} /home/blockgen/tests-${technique}.txt /home/blockgen/dot-blocktests-${technique} /home/blockgen/block-tests/blocktest /home/blockgen/block-gen/output &> ${OUTPUT_DIR}/${project_name}/docker-${technique}.log
